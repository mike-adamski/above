-- insert into CURATED_PROD.SUMMARY.IPL_ELIGIBILITY_DAILY_SUMMARY
-- (CALENDAR_DATE_CST,
-- CLIENT_ID,
-- ACTIVATION_CODE,
-- FIRST_NAME,
...
-- SERVICE_ENTITY_NAME
-- )
select CURRENT_DATE CALENDAR_DATE_CST,ipl.*,p.SOURCE_SYSTEM,p.SERVICE_ENTITY_NAME
from
(select client_id, activation_code, FIRST_NAME, LAST_NAME, Mailing_Address, CITY, STATE, ZIP_Code, Email_Address, Telephone_Number, DOB, SSN,
       ...
       nsfs_3_months, original_creditor, settlement_percent, settled_tradelined_flag, payment_adherence_ratio_3_months, payment_adherence_ratio_6_months,
...
from (
  ...
  last_nsf as (
    select program.id                         program_id,
           ...
           count(case when dateadd(month, -3, cast(CURRENT_DATE - 1 as date)) < NU_DSE_SCHEDULE_DATE_C then payment.id end) nsf_3_mos,
           --count(case when datediff(month, NU_DSE_SCHEDULE_DATE_C, cast(CURRENT_DATE - 1 as date)) <= 3 then payment.id end) nsf_3_mos,
  ...
  current_draft_amt as (
    select a.program_id
        ...
        , CASE
              WHEN datediff(MONTH, ENROLLED_DATE_CST, current_date) <= 3 THEN ((POWER(1 + (0.27 / 12), 55) - 1)) /
                                                                              ((0.27 / 12) * (POWER(1 + (0.27 / 12), 55)))
              WHEN datediff(MONTH, ENROLLED_DATE_CST, current_date) = 4 THEN ((POWER(1 + (0.256 / 12), 55) - 1)) /
                                                                             ((0.2560 / 12) * (POWER(1 + (0.2560 / 12), 55)))
              WHEN datediff(MONTH, ENROLLED_DATE_CST, current_date) = 5 THEN ((POWER(1 + (0.2435 / 12), 55) - 1)) /
                                                                             ((0.2435 / 12) * (POWER(1 + (0.2435 / 12), 55)))
              WHEN datediff(MONTH, ENROLLED_DATE_CST, current_date) >= 6 THEN ((POWER(1 + (0.229 / 12), 55) - 1)) /
...
--    left join CURATED_PROD.crm.program p on P.Is_CURRENT_RECORD_FLAG=TRUE and P.PROGRAM_NAME =a.PROGRAM_NAME
  ...
  schedule_adherence as (
    ...
    from (
      select distinct program.id program_id,
           sum(case when payment.NU_DSE_SCHEDULE_DATE_C < CURRENT_DATE and nu_dse_transaction_status_c not in ('Failed', 'Cancelled', 'Scheduled')
               then 1 else 0 end) over(partition by program_id) as successful_payments,
           sum(case when payment.NU_DSE_SCHEDULE_DATE_C < CURRENT_DATE and nu_dse_transaction_status_c in ('Failed', 'Cancelled')
               then 1 else 0 end) over(partition by program_id) as incomplete_payments,
           sum(case when payment.NU_DSE_SCHEDULE_DATE_C < CURRENT_DATE
  ...
  creditor_settlements as (
    ...
    from (
          ...
          left join CURATED_PROD.crm.creditor c_orig on t.original_creditor_id = c_orig.creditor_id
              and c_orig.is_current_record_flag
          left join CURATED_PROD.crm.creditor_alias ca_orig on t.original_creditor_alias_id = ca_orig.creditor_alias_id
              and ca_orig.is_current_record_flag
          left join CURATED_PROD.crm.creditor c_curr on t.current_creditor_id = c_curr.creditor_id
              and c_curr.is_current_record_flag
          left join CURATED_PROD.crm.creditor_alias ca_curr on t.current_creditor_alias_id = ca_curr.creditor_alias_id
              and ca_curr.is_current_record_flag
  ...
  cft_monthly_fees as (
      ...
      LEFT JOIN REFINED_PROD.SALESFORCE.NU_DSE_PROGRAM_C AS programs ON programs.PROSPECT_ID_C = prospects.ID
...
cft_prior_month_payment as (
  ...
  fico_score as (
    ...
    left join REFINED_PROD.salesforce.nu_dse_prospect_c_vw prospect on program.prospect_id_c = prospect.id
    left join REFINED_PROD.salesforce.nu_dse_financial_profile_c_vw fp on prospect.id = fp.nu_dse_prospect_c
    where coalesce(fp.nu_dse_credit_score_c,program.credit_score_c) is not null
  ),
  new_or_aged_book as (
    select distinct program.id as program_id
         ,case when datediff(month,program.enrolled_date_c,CURRENT_DATE) >= 6 then 'Aged Book'
...
fees_outstanding as (
    ...
    Deposit_Adherence as (
        ...
        Prg as (
            ...
            LEFT JOIN (
                ...
                LEFT Join (
                    Select PROGRAM_NAME, ENROLLED_DATE_CST FROM CURATED_PROD.CRM.PROGRAM WHERE IS_CURRENT_RECORD_FLAG = TRUE) P
  ...
  creditor_matrix_settlement_logic as (
        ...
        left join REFINED_PROD.CONFIGURATION.CREDITOR_TERMS as ct_alias_lump on ct_alias_lump.CREDITOR_BUCKET_NAME = a.CREDITOR_BUCKET_NAME
        ...
        left join REFINED_PROD.CONFIGURATION.CREDITOR_TERMS as ct_no_alias_lump on ct_no_alias_lump.CREDITOR_BUCKET_NAME = a.CREDITOR_BUCKET_NAME
        ...
        TRADELINES_W_EST_OFFER as (
          ...
          LEFT JOIN CURATED_PROD.CRM.CREDITOR C
              ON T.CURRENT_CREDITOR_ID = C.CREDITOR_ID AND C.IS_CURRENT_RECORD_FLAG
          left join REFINED_PROD.CONFIGURATION.CREDITOR_BUCKETS as cb on t.CURRENT_CREDITOR_ID = cb.CURRENT_CREDITOR_ID

          --currently eligibile tradelines
          --when matching on original alias required - When lump sum offer expected (For IPL)
          left join REFINED_PROD.CONFIGURATION.CREDITOR_TERMS as ct_alias_lump on ct_alias_lump.CREDITOR_BUCKET_NAME = cb.CREDITOR_BUCKET_NAME
          ...
          --when matching on original alias is not required - When lump sum offer expected (For IPL)
          left join REFINED_PROD.CONFIGURATION.CREDITOR_TERMS as ct_no_alias_lump on ct_no_alias_lump.CREDITOR_BUCKET_NAME = cb.CREDITOR_BUCKET_NAME
  ...
  select distinct
      ...
      '' tradeline_account_number,
      try_to_number(program.NU_DSE_ROUTABLE_PHONE_SETTLEMENT_ROUTING_NUMBER_C) cft_routing_number,
      try_to_number(program.NU_DSE_ROUTABLE_PHONE_SETTLEMENT_ACCOUNT_NUMBER_C) cft_account_number,
      case when try_to_number(NU_DSE_ROUTABLE_PHONE_SETTLEMENT_ROUTING_NUMBER_C) = 053101561 then 'WELLS FARGO BANK'
              when try_to_number(NU_DSE_ROUTABLE_PHONE_SETTLEMENT_ROUTING_NUMBER_C) = 053112505 then 'AXOS BANK'
              else null
      ...
      datediff(month,program.enrolled_date_c,CURRENT_DATE) months_since_enrollment,
  ...
  JOIN REFINED_PROD.SALESFORCE.NU_DSE_PROGRAM_C program ON prospect.id = program.prospect_id_c and program.BEDROCK_MIGRATION_TARGET_UUID_C IS NULL
  join REFINED_PROD.SALESFORCE.ACCOUNT act on act.id = program.NU_DSE_ACCOUNT_C and act.BEDROCK_MIGRATION_TARGET_UUID_C IS NULL
  left join CURATED_PROD.crm.program cp on cp.program_id=program.id and cp.is_current_record_flag=true
  ...
  and state IN ('CA','MI','TX','IN','NC','MO','AL','NM','TN','MS','MT','KY','FL','SD','AK','DC','OK','WI','NY','PA','VA','AZ','AR','UT','ID','LA','MD','NE')
  and (last_nsf.last_nsf_dt is null or last_nsf.last_nsf_dt < dateadd(month, -3, cast(CURRENT_DATE - 1 as date)))
  ...
  and not (state = 'MI' and program.CREATED_DATE_CST >= dateadd(month, -5, cast(CURRENT_DATE - 1 as date)))
  ...
  select distinct
      ...
      try_to_number(program.NU_DSE_ROUTABLE_PHONE_SETTLEMENT_ROUTING_NUMBER_C) cft_routing_number,
      try_to_number(program.NU_DSE_ROUTABLE_PHONE_SETTLEMENT_ACCOUNT_NUMBER_C) cft_account_number,
      case when try_to_number(program.NU_DSE_ROUTABLE_PHONE_SETTLEMENT_ROUTING_NUMBER_C) = 053101561 then 'WELLS FARGO BANK'
              when try_to_number(program.NU_DSE_ROUTABLE_PHONE_SETTLEMENT_ROUTING_NUMBER_C) = 053112505 then 'AXOS BANK'
              else null
              end as cft_bank_name
      ,concat(FIRST_NAME,' ',LAST_NAME) cft_account_holder_name
      ,NU_DSE_EXTERNAL_CFT_REST_ID_C external_id,
...
select client_id, activation_code, FIRST_NAME, LAST_NAME, Mailing_Address, CITY, STATE, ZIP_Code, Email_Address, Telephone_Number, BIRTHDATE, SSN, DRAFT_AMOUNT,
       ...
       payment_adherence_ratio_3_months, payment_adherence_ratio_6_months, HOLDER_S_NAME_C, BANK_NAME_C, ACCOUNT_NUMBER_C, ROUTING_NUMBER_C, TYPE_C,
       HISTORICAL_SETTLEMENT_PERCENT, BF_REMAINING_DEPOSITS
from (
  with
  last_nsf as (
...
--    group by program.id, program.name
        select program.PROGRAM_ID                         program_id,
           ...
           count(case when dateadd(month, -3, cast(CURRENT_DATE - 1 as date)) < SCHEDULED_DATE_CST then payment.TRANSACTION_ID end) nsf_3_mos,
--           count(case when datediff(month, SCHEDULED_DATE_CST, cast(CURRENT_DATE - 1 as date)) <= 3 then payment.TRANSACTION_ID end) nsf_3_mos,
           max(SCHEDULED_DATE_CST)        last_nsf_dt
    from CURATED_PROD.CRM.PROGRAM program
    join CURATED_PROD.CRM.SCHEDULED_TRANSACTION payment on payment.PROGRAM_ID = program.PROGRAM_id and payment.IS_CURRENT_RECORD_FLAG=TRUE
    where 1=1
  ...
  current_draft_amt as (
    select a.program_id
        ...
        , CASE
              WHEN datediff(MONTH, P.ENROLLED_DATE_CST, current_date) <= 3 THEN ((POWER(1 + (0.27 / 12), 55) - 1)) /
                                                                              ((0.27 / 12) * (POWER(1 + (0.27 / 12), 55)))
              WHEN datediff(MONTH, P.ENROLLED_DATE_CST, current_date) = 4 THEN ((POWER(1 + (0.256 / 12), 55) - 1)) /
                                                                             ((0.2560 / 12) * (POWER(1 + (0.2560 / 12), 55)))
              WHEN datediff(MONTH, P.ENROLLED_DATE_CST, current_date) = 5 THEN ((POWER(1 + (0.2435 / 12), 55) - 1)) /
                                                                             ((0.2435 / 12) * (POWER(1 + (0.2435 / 12), 55)))
              WHEN datediff(MONTH, P.ENROLLED_DATE_CST, current_date) >= 6 THEN ((POWER(1 + (0.229 / 12), 55) - 1)) /
                                                                              ((0.229 / 12) * (POWER(1 + (0.229 / 12), 55)))
              END AS DISCOUNT_FACTOR
    from (
        select distinct
 ...
 --   group by 1
  ...
  active_debts as (
    ...
    Select T.PROGRAM_ID,
      ...
      FROM CURATED_PROD.CRM.TRADELINE T
      LEFT JOIN CURATED_PROD.CRM.OFFER O on O.TRADELINE_NAME=T.TRADELINE_NAME and O.IS_CURRENT_RECORD_FLAG=True and O.IS_CURRENT_OFFER=TRUE and O.OFFER_ACCEPTED_DATE_CST is not null and coalesce(O.OFFER_CANCELLED_DATE_CST,O.SETTLEMENT_BUST_DATE_CST) is null
...
--          group by program.id
  ...
  schedule_adherence as (
    ...
    from (
      select distinct program.PROGRAM_ID, program.Program_name,
           sum(case when payment.SCHEDULED_DATE_CST < CURRENT_DATE and TRANSACTION_STATUS not in ('Failed', 'Cancelled', 'Returned','Scheduled')
               then 1 else 0 end) over(partition by program.PROGRAM_ID) as successful_payments,
           sum(case when payment.SCHEDULED_DATE_CST < CURRENT_DATE and TRANSACTION_STATUS in ('Failed', 'Returned','Cancelled')
               then 1 else 0 end) over(partition by program.program_id) as incomplete_payments,
           sum(case when payment.SCHEDULED_DATE_CST < CURRENT_DATE
  ...
  cft_monthly_fees as (
      ...
      FROM CURATED_PROD.CRM.PROGRAM P
      left join CURATED_PROD.CFT.ACTUAL_TRANSACTION ACT on ACT.PROGRAM_NAME=P.PROGRAM_NAME and ACT.IS_CURRENT_RECORD_FLAG=TRUE
      where ACT.TRANSACTION_STATUS='Completed'
      AND ACT.transaction_group='Fee'
      AND ACT.transaction_type='Monthly Service Fees'
      and datediff(month,TRANSACTION_DATE_CST,CURRENT_DATE) = 1 and P.IS_CURRENT_RECORD_FLAG=TRUE
...
cft_prior_month_payment as (
  ...
  new_or_aged_book as (
    select distinct program.program_id as program_id, program.PROGRAM_NAME
         ,case when datediff(month,program.ENROLLED_DATE_CST,CURRENT_DATE) >= 6 then 'Aged Book'
...
fees_outstanding as (
  ...
  CCPA_email as  (
    ...
    Select Distinct Native_ID as email FROM REFINED_PROD.SIMON_DATA.SENDGRID_UNSUBSCRIBES
    --UNION
  ...
  left join REFINED_PROD.BEDROCK.ACCOUNT_CONTACT_RELATION CoClient on CoClient.ACCOUNT_ID=PC.ACCOUNT_ID_C and CoClient.RELATIONSHIP_C='Co-Client' and CoClient.IS_DELETED=FALSE
  WHERE IS_CURRENT_RECORD_FLAG=TRUE
  GROUP BY 1
  ),
DQ as (
...
--new book bedrock
  ...
  select distinct
      ...
      tl_list.current_creditor tradeline_name,
      '' tradeline_account_number,
      try_to_number(ta.ROUTING_NUMBER_C) cft_routing_number,
      try_to_number(ta.ACCOUNT_NUMBER_C) cft_account_number,
      case when try_to_number(ta.ROUTING_NUMBER_C) = 053101561 then 'WELLS FARGO BANK'
      ...
      datediff(month,p.ENROLLED_DATE_CST,CURRENT_DATE) months_since_enrollment,
      next_draft_date.next_draft_date next_payment_date
      ,nvl(active_debts.Unsettled_Debt,0) + nvl(num_of_settlements.term_pay_balance,0) as total_amount_enrolled_debt
...
--  left join REFINED_PROD.SALESFORCE.NU_DSE_FEE_TEMPLATE_C ft ON ft.id = program.NU_DSE_FEE_TEMPLATE_C
  ...
  and program_no_cred.program_name is null
  and not (state = 'MI' and p.CREATED_DATE_CST >= dateadd(month, -5, cast(CURRENT_DATE - 1 as date)))
  and DQ_Check>=0
...
where b.amount_financed >= (case when state = 'CA' then 5000 else 1000 end) and b.amount_financed <= 71250
) ipl
