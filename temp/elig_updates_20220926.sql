  ...
  current_draft_amt as (
    select a.program_id
        ...
        , CASE
              ...
              WHEN datediff(MONTH, ENROLLED_DATE_CST, current_date) >= 6 THEN ((POWER(1 + (0.229 / 12), 57) - 1)) /
                                                                              ((0.229 / 12) * (POWER(1 + (0.229 / 12), 57)))
...
--           count(case when datediff(month, SCHEDULED_DATE_CST, cast($V_CALENDAR_DATE_CST - 1 as date)) <= 3 then payment.TRANSACTION_ID end) nsf_3_mos,
  ...
  current_draft_amt as (
    select a.program_id
        ...
        , CASE
              WHEN datediff(MONTH, P.ENROLLED_DATE_CST, current_date) <= 3 THEN ((POWER(1 + (0.27 / 12), 57) - 1)) /
                                                                              ((0.27 / 12) * (POWER(1 + (0.27 / 12), 57)))
              WHEN datediff(MONTH, P.ENROLLED_DATE_CST, current_date) = 4 THEN ((POWER(1 + (0.256 / 12), 57) - 1)) /
                                                                             ((0.2560 / 12) * (POWER(1 + (0.2560 / 12), 57)))
              WHEN datediff(MONTH, P.ENROLLED_DATE_CST, current_date) = 5 THEN ((POWER(1 + (0.2435 / 12), 57) - 1)) /
                                                                             ((0.2435 / 12) * (POWER(1 + (0.2435 / 12), 57)))
              WHEN datediff(MONTH, P.ENROLLED_DATE_CST, current_date) >= 6 THEN ((POWER(1 + (0.229 / 12), 57) - 1)) /
                                                                              ((0.229 / 12) * (POWER(1 + (0.229 / 12), 57)))
              END AS DISCOUNT_FACTOR
    from (
        select distinct
