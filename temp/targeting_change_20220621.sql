...
DELETE FROM {{ params.curated_database_name }}.SUMMARY.IPL_TARGETING_DAILY_SUMMARY WHERE CALENDAR_DATE_CST = current_date();

INSERT INTO {{ params.curated_database_name }}.SUMMARY.IPL_TARGETING_DAILY_SUMMARY
(
    CALENDAR_DATE_CST
    ,PROGRAM_NAME
    ,PROGRAM_ID
...
WITH ELIG_FILE_DATA AS (
                       SELECT DISTINCT
                            ...
                            , IFF(NEXT_PAYMENT_DATE < CURRENT_DATE, AD.ALT_DEPOSIT_DATE,
                                  NEXT_PAYMENT_DATE) AS NEXT_SCHEDULED_DEPOSIT_DATE_CST
                       ...
                       FROM {{ params.curated_database_name }}.SUMMARY.IPL_ELIGIBILITY_DAILY_SUMMARY E
   ...
   , FIVE9_PHONE_LOOKUP AS (
                           SELECT DISTINCT
                                ...
                                , last_value(TIMESTAMP)
                                             OVER (PARTITION BY TELEPHONE_NUMBER ORDER BY TIMESTAMP DESC)::TIMESTAMP AS LAST_BAD_DISPOSITION_DATE_TIME_CST
                           FROM {{ params.refined_database_name }}.FIVE9_LEGACY.CALL_LOG
                           WHERE CASE
                                     WHEN DISPOSITION IN
   ...
   , PROGRAM_TO_CREDIT_REPORT AS (
                                 ...
                                 FROM {{ params.refined_database_name }}.BEDROCK.OPPORTUNITY_TRADELINE_C OT
                                      LEFT JOIN {{ params.refined_database_name }}.BEDROCK.OPPORTUNITY AS O
                                                ON O.ID = OT.OPPORTUNITY_ID_C AND O.IS_DELETED = 'FALSE'
                                      LEFT JOIN {{ params.refined_database_name }}.BEDROCK.ACCOUNT AS A
                                                ON A.ID = O.ACCOUNT_ID AND A.IS_DELETED = 'FALSE'
                                      LEFT JOIN {{ params.refined_database_name }}.BEDROCK.PROGRAM_C AS P
                                                ON P.ACCOUNT_ID_C = A.ID AND P.IS_DELETED = 'FALSE' AND
                                                   datediff(MONTH, P.ENROLLMENT_DATE_C, current_date) < 6
                                      LEFT JOIN {{ params.refined_database_name }}.BEDROCK.CR_LIABILITY_C AS CL
                                                ON CL.ID = OT.CR_LIABILITY_ID_C
                                      LEFT JOIN {{ params.refined_database_name }}.BEDROCK.CR_C AS CR_C
   ...
   , CR_BANKRUPTCY AS (
                      ...
                      FROM PROGRAM_TO_CREDIT_REPORT AS A
                           LEFT JOIN {{ params.refined_database_name }}.BEDROCK.CR_PUBLIC_RECORD_C AS B
   ...
   , CR_REPO_FORECLOSURE AS (
                            ...
                            FROM PROGRAM_TO_CREDIT_REPORT AS A
                                 LEFT JOIN {{ params.refined_database_name }}.BEDROCK.CR_LIABILITY_C AS B
                                           ON A.CREDIT_REPORT_ID_C = B.CREDIT_REPORT_ID_C AND IS_DELETED = FALSE
                                 LEFT JOIN {{ params.refined_database_name }}.BEDROCK.CR_LIABILITY_COMMENT_C AS C
                                           ON B.ID = C.CR_LIABILITY_ID_C AND C.IS_DELETED = FALSE
                                 LEFT JOIN {{ params.refined_database_name }}.BEDROCK.CR_COMMENT_DEF_C AS D
   ...
   , TRADES_MISSING_FEES_PAYMENTS AS (
                                     SELECT DISTINCT
                                            T.TRADELINE_NAME
                                          , T.PROGRAM_NAME
                                     FROM {{ params.curated_database_name }}.CRM.TRADELINE AS T
                                          LEFT JOIN {{ params.curated_database_name }}.CFT.ACTUAL_TRANSACTION AS CP
                                          ...
                                          LEFT JOIN {{ params.curated_database_name }}.CFT.ACTUAL_TRANSACTION AS BF
                                          ...
                                          LEFT JOIN {{ params.curated_database_name }}.CRM.OFFER AS O
   ...
   , OB_DIALS AS (
                 ...
                 FROM (
                      ...
                      FROM {{ params.curated_database_name }}.CALL.CALL CALL
                           LEFT JOIN {{ params.curated_database_name }}.CALL.AGENT_SEGMENT AGENT_SEGMENT
                                     ON AGENT_SEGMENT.CALL_ID = CALL.CALL_ID
                           LEFT JOIN {{ params.curated_database_name }}.CALL.AGENT AGENT
                                     ON AGENT.AGENT_KEY = AGENT_SEGMENT.AGENT_KEY AND
                                        AGENT.IS_CURRENT_RECORD_FLAG = TRUE
                           LEFT JOIN {{ params.curated_database_name }}.CRM.PROGRAM PROGRAM ON PROGRAM.PROGRAM_ID = CALL.PROGRAM_ID
   ...
   , ALL_DATA AS (
                 SELECT D.*
                      ...
                      , CASE
                            ...
                            WHEN CLIENT_LOAN_INTEREST_STATUS IN ('Funded', 'Graduated', 'Not Interested')
                                THEN CLIENT_LOAN_INTEREST_STATUS
                            WHEN ABOVE_LOAN_STATUS IN ('FRONT_END_DECLINED', 'BACK_END_DECLINED')
                                THEN 'UW Declined'
                            WHEN CLIENT_LOAN_INTEREST_STATUS = 'Interested - Skybridge'
                            ...
                            WHEN ABOVE_LOAN_STATUS IN ('ONBOARDED') AND
                                 CLIENT_LOAN_INTEREST_STATUS NOT IN ('Funded', 'Graduated')
                                THEN 'Above - Loan In progress'
                      ...
                      , CASE
                            ...
                            WHEN F9PL.LAST_BAD_DISPOSITION_DATE_TIME_CST >= F9.LAST_BAD_DISPOSITION_DATE_TIME_CST
                      ...
                      , GREATEST(F9.LAST_BAD_DISPOSITION_DATE_TIME_CST, F9PL.LAST_BAD_DISPOSITION_DATE_TIME_CST) AS LAST_BAD_DISPOSITION_DATE_TIME_CST
                      , CASE
                 ...
                 FROM ELIG_FILE_DATA D
                      ...
                      LEFT JOIN (
                                SELECT DISTINCT
                                     ...
                                     , last_value(TIMESTAMP)
                                                  OVER (PARTITION BY SALESFORCE_ID ORDER BY TIMESTAMP DESC)::TIMESTAMP AS LAST_BAD_DISPOSITION_DATE_TIME_CST
                                FROM {{ params.refined_database_name }}.FIVE9_LEGACY.CALL_LOG
                                WHERE CASE
                                          WHEN DISPOSITION IN
                                               ('Transferred To 3rd Party', 'Transferred to Lender',
                      ...
                      LEFT JOIN {{ params.refined_database_name }}.SALESFORCE.NU_DSE_PROGRAM_C P_L
                                ON D.PROGRAM_NAME = P_L.NAME AND P_L.BEDROCK_MIGRATION_TARGET_UUID_C IS NULL
                      LEFT JOIN {{ params.refined_database_name }}.BEDROCK.PROGRAM_C P_B ON D.PROGRAM_NAME = P_B.NAME
                      LEFT JOIN (
                                SELECT *
                                FROM {{ params.refined_database_name }}.BEDROCK.PROGRAM_LOAN_C
                      ...
                      LEFT JOIN (
                                ...
                                FROM {{ params.refined_database_name }}.BEDROCK.PROGRAM_LOAN_C_ARCHIVE PLA
                                WHERE TRUE
                      ...
                      LEFT JOIN {{ params.refined_database_name }}.SALESFORCE.NU_DSE_PROSPECT_C AS PR
                     ...
                     SELECT *
                          ...
                          , CASE
                                ...
                                WHEN ABOVE_LOAN_STATUS IN ('FRONT_END_DECLINED', 'BACK_END_DECLINED', 'NO_OFFERS')
                                    OR FUTURE_LOAN_STATUS IN ('UW Declined')
                                    THEN 'Declined App Retarget'
                                WHEN FUTURE_LOAN_STATUS IN ('Not Interested', 'Withdrawn') OR
                          ...
                          , CASE
                                WHEN SOURCE_SYSTEM = 'LEGACY' AND FUTURE_LOAN_STATUS NOT IN
                                                                  ('Withdrawn', 'UW Declined', 'Unable to Contact',
                                                                   'Interested - Above',
                                                                   'Not Interested') THEN FALSE
                                WHEN FUTURE_LOAN_STATUS IN ('Withdrawn')
                                    AND CLIENT_LOAN_INTEREST_LAST_UPDATED_DATE_TIME_CST > current_date - 45
                                    THEN FALSE
                                WHEN FUTURE_LOAN_STATUS IN ('Not Interested')
                                    AND CLIENT_LOAN_INTEREST_LAST_UPDATED_DATE_TIME_CST > current_date - 90
                                    THEN FALSE
                                WHEN FUTURE_LOAN_STATUS IN ('UW Declined')
                                    AND CLIENT_LOAN_INTEREST_LAST_UPDATED_DATE_TIME_CST > current_date - 90
                                    THEN FALSE
                          ...
                          , CASE
                                ...
                                WHEN ABOVE_LOAN_STATUS IN ('EXPIRED') AND
                                     LOAN_APPLICATION_DATE_CST >= CURRENT_DATE - (28 + 45) --45 days from loan expiration
                                    THEN FALSE
                                WHEN ABOVE_LOAN_STATUS IN ('WITHDRAWN') AND LOAN_APPLICATION_DATE_CST >= CURRENT_DATE - 45
                                    THEN FALSE -- Temporarily hold retargeting cohorts out of the dialer
                                ELSE TRUE
                          ...
                          , CASE
                                ...
                                WHEN CLIENT_COHORT IN ('High Dollar Loan', 'Regular Campaign')
                                    AND OUTBOUND_DIAL_COUNT BETWEEN 6 AND 15
                                    THEN 'Medium Priority'
                                WHEN CLIENT_COHORT IN ('Not Interested Retarget') AND OUTBOUND_DIAL_COUNT <= 15
                                    THEN 'Medium Priority'
                                WHEN CLIENT_COHORT IN ('Not Interested Retarget') AND OUTBOUND_DIAL_COUNT >= 16
                                    THEN 'Low Priority'
                                WHEN CLIENT_COHORT IN ('High Dollar Loan', 'Regular Campaign') AND
                                     OUTBOUND_DIAL_COUNT >= 16
                                    THEN 'Low Priority'
                                ELSE '???'
