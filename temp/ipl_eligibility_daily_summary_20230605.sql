...
FROM (
     ...
     FROM (
          WITH TRADELINE_CAL AS (
                                ...
                                FROM (
                                     SELECT *
                                     FROM REFINED_PROD.GLUE_SERVICE_PUBLIC.ELIGIBILITY_TRADELINE_CALCS
                                         QUALIFY row_number() OVER (PARTITION BY TRADELINE_SFID ORDER BY UPDATED_AT_UTC DESC) = 1
                                     ) ETC
                                ...
                                WHERE TRUE
--                                   AND ETC.CREATED_AT_UTC::DATE = CURRENT_DATE
...
--                , sum(coalesce(PT.IPL_ESTIMATED_SETTLEMENT_FEES_C, 0)) OVER (PARTITION BY P.PROGRAM_ID) + 6 * COALESCE(EPC.CFT_MONHTLY_FEE, 0) + 6 * COALESCE(LEGAL_SERVICE_FEE, 0)
               , sum(coalesce(TC.BEYOND_FEES, 0)) OVER (PARTITION BY P.PROGRAM_ID) + 6 * COALESCE(EPC.CFT_MONHTLY_FEE, 0) + 6 * COALESCE(LEGAL_SERVICE_FEE, 0) AS ESTIMATED_BEYOND_PROGRAM_FEES
               , TA.BALANCE_C AS TOTAL_DEPOSITS
--                , PT.IPL_ESTIMATED_SETTLEMENT_PAYMENTS_C AS SETTLEMENT_AMOUNT
...
--                , COALESCE(PT.IPL_ESTIMATED_SETTLEMENT_RATE_C / 100, ROUND(DIV0(O.OFFER_AMOUNT_C, NTB.AMOUNT_C), 2), 0) AS SETTLEMENT_PERCENT
               ...
               , CASE WHEN PC.IPL_MONTHS_ENROLLED_C < 6 THEN PC.IPL_DEPOSIT_ADHERENCE_C / 100 ELSE 0 END AS PAYMENT_ADHERENCE_RATIO_3_MONTHS
               , coalesce(DA120.DEPADHERENCE, 0) AS PAYMENT_ADHERENCE_RATIO_4_MONTHS
               , CASE WHEN PC.IPL_MONTHS_ENROLLED_C >= 6 THEN PC.IPL_DEPOSIT_ADHERENCE_C / 100 ELSE 0 END AS PAYMENT_ADHERENCE_RATIO_6_MONTHS
               , BANK_ACCOUNT.HOLDER_S_NAME_C
               , BANK_ACCOUNT.BANK_NAME_C
               , NULL AS ACCOUNT_NUMBER_C
               , NULL AS ROUTING_NUMBER_C
               , BANK_ACCOUNT.TYPE_C
          ...
          FROM CURATED_PROD.CRM.PROGRAM P
               ...
               JOIN (
                    SELECT *
                    FROM REFINED_PROD.GLUE_SERVICE_PUBLIC.ELIGIBILITY_PROGRAM_CALCS
                        QUALIFY row_number() OVER (PARTITION BY PROGRAM_SFID ORDER BY CREATED_AT_UTC DESC) = 1
                    ) EPC ON EPC.PROGRAM_SFID = P.PROGRAM_ID --AND EPC.CREATED_AT_UTC::DATE = CURRENT_DATE
...
--                LEFT JOIN ENROLLMENT_CREDIT_SCORE ECS ON ECS.PROGRAM_NAME = P.PROGRAM_NAME
          WHERE 1 = 1
            ...
            AND COALESCE(EPC.ENROLLED_CREDIT_SCORE, 0) >= 510
