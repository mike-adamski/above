CREATE OR REPLACE VIEW SNO_SANDBOX.MADAMSKI.BEDROCK_ELIGIBILITY_FILE_QA_V AS
     ...
     FROM (
          WITH TRADELINE_CAL AS
              ...
              FROM CURATED_PROD.CRM.TRADELINE T
                   JOIN ASSIGNMENTS_PROD.BEDROCK.IPL_ELIGIBILITY_TRADELINE_CALCS ETC ON T.TRADELINE_ID = ETC.TRADELINE_SFID
              WHERE T.IS_CURRENT_RECORD_FLAG AND T.INCLUDE_IN_PROGRAM_FLAG
                  QUALIFY ROW_NUMBER() OVER (PARTITION BY ETC.TRADELINE_SFID ORDER BY ETC.CREATED_DATE DESC) = 1
              )
             , PAYMENT_DATES AS (
                                SELECT *
                                FROM (
...
--Finding latest period where no completed transaction has been made
             ...
             , LIFETIME_DEPOSIT_ADHERENCE AS
              ...
              SELECT S.PROGRAM_ID
                   , S.PROGRAM_NAME
                   , nvl(ACTUAL_AMOUNT, 0) / SCHEDULED_AMOUNT AS LIFETIME_PAYMENT_ADHERENCE
              FROM SCHEDULES S
             ...
             , CURRENT_DRAFT_AMT AS (
                                    SELECT A.PROGRAM_ID
                                         ...
                                         , CASE
                                               ...
                                               WHEN datediff(MONTH, ENROLLED_DATE_CST, current_date) = 5 THEN ((POWER(1 + (0.2435 / 12), 57) - 1)) /
                                                                                                              ((0.2435 / 12) * (POWER(1 + (0.2435 / 12), 57)))
                                               WHEN datediff(MONTH, ENROLLED_DATE_CST, current_date) >= 6 THEN ((POWER(1 + (0.239 / 12), 57) - 1)) /
                                                                                                               ((0.239 / 12) * (POWER(1 + (0.239 / 12), 57)))
          ...
          SELECT DISTINCT
               ...
               , PC.IPL_ESTIMATED_LOAN_AMOUNT_C AS AMOUNT_FINANCED
               , sum(TC.BEYOND_FEES) OVER (PARTITION BY P.PROGRAM_ID) AS ESTIMATED_BEYOND_PROGRAM_FEES
               , EPC.CURRENT_CFT_BALANCE AS TOTAL_DEPOSITS
               , TC.SETTLEMENT_PAYMENTS AS SETTLEMENT_AMOUNT
               , C.CREDITOR_NAME AS TRADELINE_NAME
               , '' AS TRADELINE_ACCOUNT_NUMBER
               , TA.ROUTING_NUMBER_C AS CFT_ROUTING_NUMBER
               , TA.ACCOUNT_NUMBER_C AS CFT_ACCOUNT_NUMBER
               , CASE
                     WHEN TA.ROUTING_NUMBER_C = 053101561 THEN 'WELLS FARGO BANK'
               ...
               , C.CREDITOR_NAME AS ORIGINAL_CREDITOR
               , TC.SETTLEMENT_RATE / 100 AS SETTLEMENT_PERCENT
               , TL_LIST.TRADELINE_SETTLEMENT_STATUS || ' - ' || TL_LIST.TRADELINE_SETTLEMENT_SUB_STATUS AS SETTLED_TRADELINED_FLAG
               , round(EPC.COMPLETED_DEPOSITS_AMOUNT_LAST_90_DAYS / EPC.SCHEDULED_DEPOSITS_AMOUNT_LAST_90_DAYS, 2) AS PAYMENT_ADHERENCE_RATIO_3_MONTHS
               , coalesce(DA120.DEPADHERENCE, 0) AS PAYMENT_ADHERENCE_RATIO_4_MONTHS
               , round(EPC.COMPLETED_DEPOSITS_AMOUNT_LAST_180_DAYS / EPC.SCHEDULED_DEPOSITS_AMOUNT_LAST_180_DAYS, 2) AS PAYMENT_ADHERENCE_RATIO_6_MONTHS
          ...
          FROM CURATED_PROD.CRM.PROGRAM P
               ...
               JOIN (
                    SELECT *
                    FROM ASSIGNMENTS_PROD.BEDROCK.IPL_ELIGIBILITY_PROGRAM_CALCS
                        QUALIFY ROW_NUMBER() OVER (PARTITION BY PROGRAM_SFID ORDER BY CREATED_DATE DESC) = 1
                    ) EPC ON EPC.PROGRAM_SFID = P.PROGRAM_ID
               LEFT JOIN TRADELINE_CAL TC ON TC.PROGRAM_NAME = P.PROGRAM_NAME
               LEFT JOIN REFINED_PROD.BEDROCK.ACCOUNT ACT ON ACT.ID = PC.ACCOUNT_ID_C AND ACT.IS_DELETED = FALSE
               LEFT JOIN REFINED_PROD.BEDROCK.ACCOUNT_CONTACT_RELATION ACR ON ACR.ACCOUNT_ID = ACT.ID AND ACR.RELATIONSHIP_C = 'Client' AND ACR.IS_DELETED = FALSE
               LEFT JOIN REFINED_PROD.BEDROCK.CONTACT CT ON ACR.CONTACT_ID = CT.ID AND CT.IS_DELETED = FALSE
