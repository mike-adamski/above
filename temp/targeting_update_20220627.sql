WITH ELIG_FILE_DATA AS (
   ...
   , FIVE9_PHONE_LOOKUP AS (
                           SELECT DISTINCT
                                ...
                                , last_value(TIMESTAMP_CST)
                                             OVER (PARTITION BY TELEPHONE_NUMBER ORDER BY TIMESTAMP_CST DESC)::TIMESTAMP AS LAST_BAD_DISPOSITION_DATE_TIME_CST
   ...
   , ALL_DATA AS (
                 SELECT D.*
                      ...
                      , CASE
                            WHEN F9.LAST_BAD_DISPOSITION_DATE_TIME_CST IS NULL
                                THEN F9PL.LAST_BAD_DISPOSITION_DATE_TIME_CST
                            WHEN F9PL.LAST_BAD_DISPOSITION_DATE_TIME_CST IS NULL
                                THEN F9.LAST_BAD_DISPOSITION_DATE_TIME_CST
                            ELSE GREATEST(F9.LAST_BAD_DISPOSITION_DATE_TIME_CST,
                                          F9PL.LAST_BAD_DISPOSITION_DATE_TIME_CST)
